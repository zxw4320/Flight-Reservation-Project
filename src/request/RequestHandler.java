...
/**
 * Handles all user input. Parses the input and calls the proper command, or prints the proper error
 * to the user. Fulfills the Invoker part of the Command design pattern.
 */
public class RequestHandler {
    ...
    public RequestHandler(RouteMap routeMap, AirportStorage airportStorage,
    ...
    /**
     * Handles taking in strings of input from a client. It takes arguments separated by commas and
     * terminated with a semi-colon. It stores the previous part of the requestString and will
     * accept more until it receives the terminator.
     *
    ...
    public void makeRequest(ui.AFRSInterface ui, String requestString) {
        ...
        switch (command) {
            ...
            case "airport":
                ...
                break;
            case "server":
                parseServer(ui, requestArray);
                break;
            case "undo":
                requestHistory.undo(ui);
                break;
            case "redo":
                requestHistory.redo(ui);
                break;
            default:
                ui.printString("error,unknown request");
                break;
        }
    ...
    /**
     * Checks for the sort order and generates a FlightInfoRequest with the proper order or default
     * if none is specified. Executes the request.
     *
    ...
    private void parseAirport(ui.AFRSInterface ui, ArrayList<String> argumentArray) {
        // create request
        Request airportRequest = new AirportInfoRequest(ui, routeMap, airportStorage,
            argumentArray.get(1));
        // execute request
